...
var (
	...
	offset   = flag.Int("o", 0, "offset in file, 0 based")
...
func pkgFiles(p string) []string {
	...
	out := make([]string, len(pkg.GoFiles))
...
func main() {
	...
	*filename, _ = filepath.Abs(*filename)
	...
	lg("target is %v@%s", target, printPos(target.Pos()))
...
func twoPass(myPkg string, fs []*ast.File, target *ast.Ident) {
	...
	obj := info.Uses[target]
	if obj == nil {
		log.Fatalf("object of target=%v not found", target)
	}
